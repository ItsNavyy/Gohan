...
def Tools_Menu():
    global send_attack
    Tools = [
        [sg.Text("Target"), sg.Input(key="IP")],
    ...
    windows = sg.Window('Gohan DDoS by Par4xy & Ryo', Tools, grab_anywhere=True, font=("Helvetica", 12), auto_size_buttons=False, keep_on_top=True, disable_minimize=True)
                ...
                if args["TCP"] == True:
                    for i in range(101):
                        ...
                        if i == 100:
                            ...
                            save_log(f"Started Flooding | {IP}:{Port} | {Threads} | {Time} Seconds\n")
                            windows["log"].update(open("logs.txt").read()) # update log
                            break
